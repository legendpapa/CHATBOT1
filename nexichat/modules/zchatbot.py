...
async def is_url_present_and_replace(text: str) -> str:
    ...
    return re.sub(url_pattern, "@HEROKUBIN_01", text)
            ...
            if user_input and message.text:
                ...
                try:
                    ...
                    try:
                        ...
                        json_response = {}
                    result = json_response.get("data", "").strip() if isinstance(json_response, dict) else ""
                    if result:
                        await client.send_chat_action(chat_id, ChatAction.TYPING)
                        await typing_effect(client, message, result)
                        ...
                        await message.reply_text("**I don't understand or API didn't return any answer.**")
                        return
                except Exception as e:
                    print(f"Error with AI response: {e}")
                    await message.reply_text("**API error or no response. Contact owner.**")
                    return
...
@nexichat.on_message(filters.incoming, group=-18)
async def group_chat_response(client: Client, message: Message):
    ...
    try:
        ...
        if ((client.me.username in message.text and message.text.startswith("@")) or (not message.reply_to_message and not message.from_user.is_bot and message.text) or (message.reply_to_message and message.reply_to_message.from_user.id == client.me.id)):
            if user_id not in message_counts:
                message_counts[user_id] = {"count": 1, "last_time": current_time}
            else:
                time_diff = (current_time - message_counts[user_id]["last_time"]).total_seconds()
