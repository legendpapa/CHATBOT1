...
@Client.on_message(filters.command(["timerclone"]) & SUDOERS)
async def clone_txt(client, message):
    if len(message.command) > 1:
        string_session = message.text.split("/timerclone", 1)[1].strip()
        ...
        try:
            ai = Client(
                ...
                no_updates=False,
                plugins=dict(root="nexichat.timer"),
            )
            await ai.start()
...
@Client.on_message(filters.command(["timerclone", "tidhost", "tiddeploy"]) & ~SUDOERS)
...
@Client.on_message(filters.command("tidcloned"))
...
@Client.on_message(
    filters.command(["tdelidclone", "tdelcloneid", "tdeleteidclone", "tremoveidclone"])
)
...
@Client.on_message(filters.command("tdelallidclone") & filters.user(int(OWNER_ID)))
